...
// Create New Ticket
export const formTicket = createAsyncThunk('tickets/create', async(ticketData, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token
        return await ticketService.formTicket(ticketData, token)
    } catch (error) {
        const message = 
        (error.response &&
          error.response.data &&
...
export const ticketSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
         .addCase(formTicket.pending, (state) => {
            state.isLoading = true
         })
         .addCase(formTicket.fulfilled, (state) => {
            state.isLoading = false
            state.isSuccess = true
         })
         .addCase(formTicket.rejected, (state, action) => {
            state.isLoading = false
            state.isError = true
